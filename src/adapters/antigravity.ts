...
`;
    ...
    for (const [filename, content] of Object.entries(commands)) {
      // e.g. filename = 'wlp/propose.md' -> baseName = 'wlp/propose'
      const baseName = filename.replace('.md', '');
      
      // Folder name: 'wlp-propose' (flat structure required by Antigravity)
      const folderName = baseName.replace(/\//g, '-');
      // Skill name for UI autocomplete: 'wlp:propose'
...
`;
    ...
    if (count > 0) {
      console.log(pc.green('✓ Created .agents/skills/ proxy skills for Google Antigravity'));
    } else {
